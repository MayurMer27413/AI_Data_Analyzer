...
"""
    ...
    try:
        ...
        if clean_code.startswith("df") or "df[" in clean_code or "df." in clean_code or "len(df" in clean_code:
            ...
            # Mitigate sandbox escapes by blocking dangerous terms
            if any(forbidden in clean_code for forbidden in ["__", "import", "os", "sys", "open"]):
                return "🛡️ Security Block: Unsafe or unrecognized command generated by AI."
                
            try:
                # Safely evaluate the pandas expression without OS builtins
                eval_locals = {"df": df, "pd": pd}
                result = eval(clean_code, {"__builtins__": {}}, eval_locals)
