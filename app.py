...
def _tab_gallery():
    ...
    for i, chart_data in enumerate(st.session_state.saved_charts):
        ...
        col_title, col_check = st.columns([10, 2])
        with col_title:
            st.markdown(f"##### {chart_data['name']}")
        with col_check:
            st.markdown("<div style='padding-top:0.35rem'></div>", unsafe_allow_html=True)
            include = st.checkbox(
                "Include", key=f"chk_{safe_name}_{i}", value=True,
                help="Include this chart in the PDF report"
            )
        st.plotly_chart(
            chart_data["fig"], use_container_width=True,
            key=f"gallery_chart_{safe_name}_{i}"
        )
        if include:
    ...
    if st.button("Generate PDF Report", type="primary"):
        ...
        with st.spinner("Generating PDF…"):
            ...
            pdf_css = (
                ...
                "color: #0F172A; margin: 40px; }"
                "h1 { color: #4F46E5; font-size: 24px; margin-bottom: 4px; }"
                "h2 { color: #334155; font-size: 18px; margin-top: 28px; }"
            ...
            html = (
                ...
                f"</head><body>"
                f"<h1>AI Data Analysis Report</h1>"
                f"<p class='note'>Generated by AI Data Analyzer</p><hr/>"
            )

            # ── AI Insights section ─────────────────────────────────────────
            if "insights" in st.session_state:
                html += "<h2>AI-Generated Insights</h2>"
                html += markdown.markdown(st.session_state.insights)
                html += "<hr/>"

            # ── Charts section (listed by name — no kaleido needed) ─────────
            if selected_indices:
                html += "<h2>Visualizations Included</h2>"
                html += (
                    "<p class='note'>"
                    "Interactive charts are available in the app. "
                    "Chart titles are listed below for reference."
                    "</p>"
                )
                for idx in selected_indices:
                    chart = st.session_state.saved_charts[idx]
                    html += f"<div class='chart-item'>📊 {chart['name']}</div>"
                html += "<hr/>"

            html += (
                "<p class='footer'>This report was generated automatically "
                "by the AI Data Analyzer platform.</p>"
            )
            html += "</body></html>"
